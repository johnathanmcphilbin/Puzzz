...
serve(async req => {
  ...
  try {
    ...
    if (!supabaseAnonKey) {
      ...
      throw new Error(
        'Supabase configuration error. Please contact the administrator.'
      );
    }

    const { roomCode, customization, crazynessLevel, gameType } =
      await req.json();
    ...
    console.log('Generating room questions for:', {
      roomCode,
      customization,
      crazynessLevel,
      gameType,
    ...
    const roomCheckResponse = await fetch(
      `${functionsUrl}/rooms-service?roomCode=${roomCode}`,
      {
        method: 'GET',
        headers: {
    ...
    if (!roomCheckResponse.ok) {
      ...
      console.error('Room check failed:', {
    ...
    if (gameType === 'would_you_rather') {
      ...
      ${crazynessConfig.examples
        .filter(e => e.includes('Would You Rather'))
        .map(e => `• ${e}`)
        .join('\n')}
      ...
      EXAMPLES OF APPROPRIATE CONTENT FOR THIS LEVEL:
      ${crazynessConfig.examples
        .filter(e => e.includes('Odd One Out'))
        .map(e => `• ${e}`)
        .join('\n')}
    ...
    try {
      ...
      if (Array.isArray(questions.questions)) {
        console.log(
          `Parsed ${questions.questions.length} questions from OpenAI response`
        );
      }
    } catch (parseError) {
    ...
    if (gameType === 'would_you_rather') {
      ...
      console.log(
        '🔵 [ROOM-QUESTIONS] aiQuestions to store:',
      ...
      console.log(
        '🟡 [ROOM-QUESTIONS] Update payload being sent:',
      ...
      console.log(
        '🟡 [ROOM-QUESTIONS] Update response status:',
      ...
      console.log(
        '🟢 [ROOM-QUESTIONS] Update successful, result:',
        JSON.stringify(updateResult, null, 2)
      );

      console.log(
        `Successfully stored ${questions.questions?.length || 0} Would You Rather AI questions for room ${roomCode} with ${crazynessLevel}% craziness`
      );
    } else {
      ...
      if (!updateResponse.ok) {
        ...
        throw new Error(
          `Failed to store questions in Redis: ${errorData.error || 'Unknown error'}`
        );
      }
    }
    ...
    const gameDisplayName =
      gameType === 'would_you_rather'
        ...
        : gameType === 'paranoia'
          ? 'Paranoia'
          : gameType === 'odd_one_out' || gameType === 'odd-one-out'
            ? 'Odd One Out'
            : gameType;

    console.log(
      `Successfully stored ${questions.questions?.length || 0} ${gameDisplayName} questions for room ${roomCode} with ${crazynessLevel}% craziness`
    );
