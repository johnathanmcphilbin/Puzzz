...
function addImportsIfNeeded(content) {
  const needsFunctionsImport = content.includes('FUNCTIONS_BASE_URL') || content.includes('SUPABASE_ANON_KEY');
  ...
  if (needsFunctionsImport && !hasFunctionsImport) {
    ...
    if (lastImportIndex !== -1) {
      lines.splice(lastImportIndex + 1, 0, "import { FUNCTIONS_BASE_URL, SUPABASE_ANON_KEY } from '@/utils/functions';");
      return lines.join('\n');
    }
  }
...
const migrations = [
  ...
  {
    pattern: /const characterIds = players\.map\(p => p\.selected_character_id\)\.filter\(Boolean\);/g,
    replacement: 'const characterIds = players.map(p => p.selected_character_id).filter((id): id is string => Boolean(id));'
  },

  // Fix questions data mapping to handle nulls
  {
    pattern: /questions = \(questionsData \|\| \[\]\)\.map\(q => \(\{\s*\.\.\.q,\s*category: q\.category \|\| "general"\s*\}\)\);/g,
  ...
  {
    pattern: /const \{ error \} = await supabase\s*\.from\("rooms"\)\s*\.update\(\{ game_state: ([^}]+) \}\)\s*\.eq\("id", room\.id\);/g,
  ...
  {
    pattern: /const \{ error: insertError \} = await supabase\s*\.from\("forms_responses"\)\s*\.insert\([^)]+\);\s*if \(insertError\) throw insertError;/g,
  ...
  {
    pattern: /await supabase\s*\.from\("game_votes"\)\s*\.(delete|insert)\([^}]+\}?\);/g,
  ...
  {
    pattern: /const \{ data: roomQuestions[^}]+\} = await supabase\s*\.from\("room_questions"\)[^;]+;/g,
    replacement: '// Room questions are no longer used - using default questions'
  },

  // Replace ai_chat_customizations operations (remove them since table is deleted)
  {
    pattern: /const \{ data: customizationData[^}]+\} = await supabase\s*\.from\("ai_chat_customizations"\)[^;]+;/g,
    replacement: '// AI customizations are no longer stored in database'
  },

  // Replace players table operations with Redis operations
  {
    pattern: /await supabase\s*\.from\("players"\)\s*\.update\([^)]+\)\s*\.eq\("room_id", room\.id\)\s*\.eq\("player_id", [^)]+\);/g,
    replacement: `// Player updates are now handled through Redis room data`
  },

  // Replace players table deletes with Redis operations
  {
    pattern: /await supabase\s*\.from\("players"\)\s*\.delete\(\)\s*\.eq\("room_id", room\.id\)\s*\.eq\("player_id", [^)]+\);/g,
  ...
  // Fix results calculation null checks
  {
    ...
    replacement: 'if (results[question.id]) results[question.id][player.player_id] = 0;'
  },

  {
    pattern: /results\[questionId\]\[selectedPlayerId as string\]\+\+;/g,
    replacement: 'if (results[questionId]) results[questionId][selectedPlayerId as string]++;'
  },

  // Fix winner votes access
  {
    pattern: /const winnerVotes = questionResults\[winner\?\.player_id\] \|\| 0;/g,
    replacement: 'const winnerVotes = winner ? (questionResults[winner.player_id] || 0) : 0;'
  },

  // Remove submitResponses database operations for Forms game
  {
    pattern: /const \{ error \} = await supabase\s*\.from\("rooms"\)\s*\.update\(\{ game_state: updatedGameState \}\)\s*\.eq\("id", room\.id\);\s*if \(error\) throw error;/g,
  ...
  {
    pattern: /const \{ error: resultsError \} = await supabase\s*\.from\("rooms"\)\s*\.update\(\{ game_state: resultsGameState \}\)\s*\.eq\("id", room\.id\);/g,
  ...
  {
    pattern: /const \{ error \} = await supabase\s*\.from\("rooms"\)\s*\.update\(\{\s*game_state: \{ phase: "lobby" \}\s*\}\)\s*\.eq\("id", room\.id\);/g,
    replacement: `const response = await fetch(\`\${FUNCTIONS_BASE_URL}/rooms-service\`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'apikey': SUPABASE_ANON_KEY, 'Authorization': \`Bearer \${SUPABASE_ANON_KEY}\` },
        body: JSON.stringify({ 
...
function migrateFile(filePath) {
  ...
  for (const migration of migrations) {
    ...
    if (content !== before) {
      modified = true;
      console.log(`  ✓ Applied pattern: ${migration.pattern.source.substring(0, 50)}...`);
    }
  }
...
function runMigration() {
  ...
  console.log('  3. Run the database migration: supabase/migrations/20250729004221_remove_room_database_tables.sql');
}
